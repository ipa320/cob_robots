...
from control_msgs.msg import JointControllerState, JointTrajectoryControllerState, FollowJointTrajectoryActionGoal, FollowJointTrajectoryActionResult
...
class cob_controller_adapter_gazebo():
  ...
  def __init__(self):
    ...
    self.joint_trajectory_controller_state_pub = rospy.Publisher("state", JointTrajectoryControllerState, queue_size=1)
  ...
  def cmd_vel_cb(self, data):    
    ...
    if self.current_control_mode != "velocity":
      ...
      self.switch_controller(self.vel_controller_names, self.trajectory_controller_names)
      self.current_control_mode = "velocity"
      print("Current control mode: "+self.current_control_mode)
      #print data    
    for i in range(len(self.joint_names)):
