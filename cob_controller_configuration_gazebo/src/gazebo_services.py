...
class gazebo_services():
	...
	def __init__(self):
		self.action_client = actionlib.SimpleActionClient('joint_trajectory_controller/follow_joint_trajectory', FollowJointTrajectoryAction)
		self.init_srv = rospy.Service('driver/init', Trigger, self.init_cb)
		self.stop_srv = rospy.Service('driver/stop', Trigger, self.stop_cb)
		self.recover_srv = rospy.Service('driver/recover', Trigger, self.recover_cb)
		self.set_operation_mode_srv = rospy.Service('driver/set_operation_mode', SetOperationMode, self.set_operation_mode_cb)
